...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (data.id == null || data.name == null || data.description == null || selectedColor == null) {
	...
	try {
		...
		if (categoryWithExistingName != null && category.id !== categoryWithExistingName.id) {
			res.status(400).json({ message: 'Existing name' });
			return;
		}

		await Category.editCategory(data.id, data.name, data.description, selectedColor);
