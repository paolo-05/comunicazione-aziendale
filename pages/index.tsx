...
export default function Home(): React.ReactElement {
	...
	const { posts } = useAllVisiblePosts();
			...
			return (
				<div
					...
					className={`flex flex-col items-center justify-center rounded-lg p-6 ${
						post.actualDate == actualDate
							? 'bg-primary-700 text-white font-bold'
							: 'bg-gray-100 max-w-lg dark:bg-gray-900'
					}`}
				>
		...
		<>
			...
			<main className={inter.className}>
				...
				<section className='min-h-screen flex items-center justify-center'>
					<Container>
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4'>{renderPosts()}</div>
					</Container>
				</section>
			</main>
		</>
